...
function AddressInput({ value, onChange, onSelect }) {
  ...
  async function handleChange(val) {
    ...
    if (!val || val.length < 1) { setSuggestions([]); setOpen(false); return; }
    timer.current = setTimeout(async () => {
      try {
        const res = await fetch(`/api/address?q=${encodeURIComponent(val)}&limit=6`);
  ...
  return (
    <div ref={ref} style={{ position: 'relative' }}>
      <span style={{ position: 'absolute', left: 13, top: '50%', transform: 'translateY(-50%)', pointerEvents: 'none', zIndex: 2, fontSize: 15, color: '#a07820' }}>📍</span>
      <input
        type="text" value={value} autoComplete="off"
        onChange={e => handleChange(e.target.value)}
        placeholder="Ex: 12 avenue des Fleurs, 75011 Paris"
        ...
        <div style={{ position: 'absolute', top: 'calc(100% + 4px)', left: 0, right: 0, zIndex: 99999, background: '#0e0e1a', border: '0.5px solid rgba(160,120,32,.4)', borderRadius: 10, overflow: 'hidden', boxShadow: '0 12px 48px rgba(0,0,0,.9)' }}>
          {suggestions.map((s, i) => (
            <button key={i} type="button"
              onMouseDown={e => { e.preventDefault(); onSelect(s.label); setOpen(false); setSuggestions([]); }}
              style={{ width: '100%', display: 'flex', flexDirection: 'column', padding: '11px 14px', background: 'transparent', border: 'none', borderBottom: i < suggestions.length - 1 ? '0.5px solid #111118' : 'none', cursor: 'pointer', textAlign: 'left', fontFamily: 'inherit' }}
...
export default function AnalysePage() {
  ...
  const downloadRapport = () => {
    if (!result) return;
    const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>Rapport PLU — PermitAI</title><style>body{font-family:Arial,sans-serif;padding:40px;color:#333;max-width:800px;margin:0 auto}h1{color:#a07820;border-bottom:2px solid #a07820;padding-bottom:10px}.rule{padding:10px 12px;border-left:3px solid #4ade80;margin:6px 0;background:#f0fff4}.footer{margin-top:48px;padding-top:16px;border-top:1px solid #eee;font-size:11px;color:#aaa}</style></head><body><h1>📋 Rapport PLU — PermitAI</h1><p><strong>Adresse :</strong> ${adresse}</p><p><strong>Commune :</strong> ${result.commune||''}</p><p><strong>Zone :</strong> ${result.zone||'UB'}</p><p><strong>Score :</strong> ${result.score_confiance||87}%</p>${(result.regles_applicables||result.rules||[]).map(r=>`<div class="rule"><strong>${r.article}</strong><br>${r.contenu||r.detail||''}</div>`).join('')}<div class="footer">PermitAI · permitai.eu · ${new Date().toLocaleDateString('fr-FR')}</div></body></html>`;
    const blob = new Blob([html],{type:'text/html;charset=utf-8'});
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a'); a.href=url; a.download=`rapport-plu-${new Date().toISOString().split('T')[0]}.html`;
  ...
  async function analyser() {
    ...
    try {
      const res = await fetch('/api/plu/query', { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify({ adresse, address: adresse, description, type: typeTravauxValue, surface }) });
      const data = await res.json();
    ...
    <div style={{ minHeight:'100vh', background:'#06060e', fontFamily:"'DM Sans', sans-serif" }}>
      <nav style={{ padding:'14px 52px', borderBottom:'0.5px solid #1c1c2a', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
        <Link href="/" style={{ display:'flex', alignItems:'center', gap:8, textDecoration:'none' }}>
          <div style={{ width:28, height:28, background:'#a07820', borderRadius:7, display:'flex', alignItems:'center', justifyContent:'center' }}><svg width="12" height="12" viewBox="0 0 13 13" fill="none"><path d="M1 5.5L7 1L13 5.5V13H1V5.5Z" stroke="white" strokeWidth="1.3"/><rect x="4.5" y="8" width="4" height="5" rx=".4" fill="white"/></svg></div>
      ...
      <div style={{ padding:'48px 52px 64px', maxWidth:1100, margin:'0 auto' }}>
        ...
        <h1 style={{ fontSize:38, fontWeight:500, color:'#f2efe9', marginBottom:6, letterSpacing:'-0.5px', lineHeight:1.1 }}>Analysez votre projet<br/><em style={{ fontStyle:'italic', color:'#e8b420' }}>en 3 minutes.</em></h1>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'400px 1fr', gap:18 }}>
          <div style={{ background:'#0e0e1a', border:'0.5px solid #1c1c2a', borderRadius:14, padding:24 }}>
            ...
            <div style={{ marginBottom:13 }}>
              <AddressInput value={adresse} onChange={setAdresse} onSelect={v => setAdresse(v)} />
            </div>
            ...
            <textarea value={description} onChange={e => setDescription(e.target.value)} placeholder="Extension de 40m² plain-pied avec véranda vitrée..." rows={4} style={{ ...iStyle, resize:'none', marginBottom:13 }} />
            ...
            {parseInt(surface) > 150 && (
              <div style={{ padding:'10px 12px', background:'rgba(239,68,68,.06)', border:'0.5px solid rgba(239,68,68,.2)', borderRadius:8, fontSize:11, color:'#ef4444', marginBottom:12 }}>
                ⚖️ <strong>Architecte obligatoire</strong> — Surface {'>'} 150m² (Loi du 3 janvier 1977)
              </div>
            )}
            ...
            {error && <div style={{ marginTop:10, padding:'9px 12px', background:'rgba(239,68,68,.07)', border:'0.5px solid rgba(239,68,68,.2)', borderRadius:8, fontSize:12, color:'#ef4444' }}>{error}</div>}
            ...
            {result ? (
              <div>
                ...
                <div style={{ padding:'13px 15px', background:'rgba(74,222,128,.07)', border:'0.5px solid rgba(74,222,128,.2)', borderRadius:9, marginBottom:14 }}>
                  ...
                  <p style={{ fontSize:12.5, color:'#8d887f', lineHeight:1.62, margin:0 }}>{result.resume||'Projet globalement conforme aux règles PLU.'}</p>
                </div>
                ...
                <div style={{ textAlign:'center', marginTop:-30, marginBottom:16, position:'relative', zIndex:2 }}>
                  <Link href="/tarifs"><button style={{ padding:'10px 20px', background:'linear-gradient(90deg,#a07820,#c4960a)', border:'none', borderRadius:9, color:'#fff', fontSize:12, fontWeight:600, cursor:'pointer', fontFamily:'inherit' }}>Débloquer — Starter 29€/mois</button></Link>
                </div>
                {(result.cerfa_recommande||result.cerfa) && (
                  <div style={{ background:'#111118', border:'0.5px solid #1c1c2a', borderRadius:8, padding:'12px 14px', marginBottom:12 }}>
                    ...
                    <div style={{ fontSize:13, fontWeight:500, color:'#f2efe9', marginBottom:8 }}>{result.cerfa_recommande||result.cerfa?.numero||'13406'} — {result.cerfa?.nom||'Permis de construire MI'}</div>
                    <div style={{ display:'flex', gap:8 }}>
                      <button onClick={downloadRapport} style={{ flex:1, padding:'9px', background:'transparent', border:'0.5px solid rgba(160,120,32,.3)', borderRadius:8, color:'#a07820', fontSize:11, fontWeight:500, cursor:'pointer', fontFamily:'inherit' }}>⬇ Télécharger le rapport</button>
                      <Link href={`/cerfa/wizard?cerfa=${result.cerfa_recommande||'13406'}&adresse=${encodeURIComponent(adresse)}&commune=${encodeURIComponent(result.commune||'')}&surface=${surface}&type=${typeTravauxValue}`} style={{ flex:1, textDecoration:'none' }}>
                        <button style={{ width:'100%', padding:'9px', background:'linear-gradient(90deg,#a07820,#c4960a)', border:'none', borderRadius:8, color:'#fff', fontSize:11, fontWeight:600, cursor:'pointer', fontFamily:'inherit' }}>📋 Remplir le CERFA →</button>
                      </Link>
                    </div>
                  </div>
                )}
                <Link href={`/depot?adresse=${encodeURIComponent(adresse)}&commune=${encodeURIComponent(result.commune||'')}`} style={{ textDecoration:'none', display:'block' }}>
                  <button style={{ width:'100%', padding:'10px', background:'transparent', border:'0.5px solid #1c1c2a', borderRadius:9, color:'#5a5650', fontSize:12, cursor:'pointer', fontFamily:'inherit' }}>📬 Déposer ce dossier en mairie →</button>
                </Link>
              </div>
            ) : (
              <div style={{ height:'100%', minHeight:360, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:14, textAlign:'center' }}>
                ...
                {['Paris, Lyon, Marseille, Bordeaux','Toulouse, Nantes, Rennes, Nice','Et 34 970 communes de France'].map((t,i) => (
                  <div key={i} style={{ fontSize:10, color:'#3e3a34', display:'flex', alignItems:'center', gap:5 }}><div style={{ width:4, height:4, borderRadius:'50%', background:'#a07820' }}/>{t}</div>
                ))}
              </div>
            )}
          </div>
        ...
        <div style={{ marginTop:16, background:'#0a0a14', border:'0.5px solid #1c1c2a', borderRadius:11, padding:'14px 20px', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
          <div style={{ display:'flex', gap:32 }}>
            {[['34 970','communes indexées'],['3 min',"temps d'analyse"],['94%','taux acceptation'],['13','CERFA disponibles']].map(([v,l]) => (
              <div key={l} style={{ display:'flex', alignItems:'center', gap:8 }}><span style={{ fontSize:18, color:'#e8b420', fontWeight:500 }}>{v}</span><span style={{ fontSize:11, color:'#5a5650' }}>{l}</span></div>
            ))}
          </div>
